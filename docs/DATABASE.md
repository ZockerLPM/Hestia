...
model User {
  ...
  // Personalisierung (Wand-Modus + Pendel/Wetter)
  homeLat       Float?
  homeLng       Float?
  homeLabel     String?
  workLat       Float?
...
| `homeLat/Lng/Label` | Wohnort/Heimatbahnhof — für Pendel-Anzeige im Wand-Modus |
| `workLat/Lng/Label` | Arbeit/Zielbahnhof |
| `commuteMode` | Bestimmt, welche Pendel-Karte gezeigt wird: ÖV-Verbindung (transit), Stau (driving), keine Provider-Calls (biking/walking) |
| `weatherLat/Lng` | Optionaler Wetter-Standort; falls null wird home verwendet |
...
Ein registriertes Gesicht für die Wand-Erkennung. Pro Person können
mehrere Descriptors gespeichert sein (verschiedene Winkel, Beleuchtung,
mit/ohne Brille).

- `descriptor` ist ein JSON-stringifiziertes 128-Float-Array — der
  Embedding-Vektor von `@vladmandic/face-api`. **Nicht rückkonvertierbar**
  in ein Foto.
- Beim Erkennen lädt der Wand-Modus alle Descriptors und nutzt
  `FaceMatcher` mit Distance-Threshold `0.55` (höher = laxer).
- `onDelete: Cascade` — beim User-Delete fliegen Descriptors mit.
...
Arbeitsschichten/Termine, die im Personal-Panel der Wand erscheinen.
Bewusst getrennt vom `CalendarEvent`-Model:
- Schichten sind **privat** pro User (CalendarEvent teilt der Haushalt)
- Andere Datenform (Start+Ende statt allDay, kein Color/Recurrence)

`@@index([userId, startsAt])` für schnelle "nächste Schicht"-Query.
...
Wiederkehrende Schicht-Muster, z.B. "jeden Montag 08:00–17:00". Ein täglicher
Cron-Job (03:00) generiert aus aktiven Patterns konkrete `WorkShift`-Einträge
für den 14-Tage-Horizont. Duplikate werden anhand von `userId + startsAt + endsAt`
erkannt und übersprungen.

- `weekday`: 0=Montag … 6=Sonntag (ISO-Wochentagsreihenfolge)
- `validFrom` / `validUntil`: optionaler Gültigkeitszeitraum (z.B. Saisonarbeit)
- Kann auch via `POST /shifts/patterns/generate` manuell ausgelöst werden
...
Mood-Check-in der Wall-Personalisierung. Wird einmalig pro Tag nach dem
Gesichts-Login angezeigt. Verlauf via `GET /mood` (30 Tage) für zukünftige
...
Persönliche Dashboard-Konfiguration pro User. Das JSON-Schema:

```typescript
interface WallConfig {
  cards: Array<{
    id: string;        // 'tasks' | 'events' | 'meals' | 'shopping' | 'expiring'
                       // | 'upcoming-tasks' | 'budget' | 'countdown'
    enabled: boolean;
    order: number;
    wide?: boolean;    // ob Card 2 Spalten belegt
  }>;
  bgColor?: string;    // Hex-Farbe, default '#030712'
  showSeconds?: boolean; // Uhr zeigt HH:MM:SS statt HH:MM
}
```
...
User ──┬─────► FinanceEntry ──► FinanceCategory ◄── RecurringFinance
       ...
       │
       ├─────► FaceDescriptor   (Wand-Erkennung)
       │
       ├─────► WorkShift        (konkrete Schichten)
       │
       ├─────► ShiftPattern     (Wochenmuster → generiert WorkShifts)
       │
       ├─────► MoodLog          (täglicher Check-in)
       │
       └─────► WallConfig       (pro-User Dashboard-Layout)
